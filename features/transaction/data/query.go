...
func (tq *transactionQuery) GetTransactionHistory(userId uint, status, from, to string) ([]transaction.Core, error) {
	...
	var err error
	if from == "" && to == "" {
		err = tq.db.Raw("SELECT t.id , c.id customer_id , c.name customer_name , total_price , t.created_at , transaction_status , invoice_number , invoice_url , payment_url FROM transactions t JOIN customers c ON t.user_id = c.id WHERE t.user_id = ? AND product_status = '?'", userId, status).Scan(&trans).Error
	} else if to == "" {
		err = tq.db.Raw("SELECT t.id , c.id customer_id , c.name customer_name , total_price , t.created_at , transaction_status , invoice_number , invoice_url , payment_url FROM transactions t JOIN customers c ON t.user_id = c.id WHERE t.user_id = ? AND product_status = '?' AND t.created_at >= ?", userId, status, from).Scan(&trans).Error
	} else if from == "" {
		err = tq.db.Raw("SELECT t.id , c.id customer_id , c.name customer_name , total_price , t.created_at , transaction_status , invoice_number , invoice_url , payment_url FROM transactions t JOIN customers c ON t.user_id = c.id WHERE t.user_id = ? AND product_status = '?' AND t.created_at <= ?", userId, status, to).Scan(&trans).Error
	} else {
		err = tq.db.Raw("SELECT t.id , c.id customer_id , c.name customer_name , total_price , t.created_at , transaction_status , invoice_number , invoice_url , payment_url FROM transactions t JOIN customers c ON t.user_id = c.id WHERE t.user_id = ? AND product_status = '?' AND t.created_at >= ? AND t.created_at <= ?", userId, status, from, to).Scan(&trans).Error
	}
	if err != nil {
