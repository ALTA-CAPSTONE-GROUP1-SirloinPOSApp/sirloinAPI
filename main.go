...
func main() {
	...
	e.POST("/regsiter_device", userHdl.RegisterDevice(), echojwt.JWT([]byte(config.JWT_KEY)))
	...
	e.PUT("/transactions/:transaction_id", transHdl.UpdateStatus(), echojwt.JWT([]byte(config.JWT_KEY)))
